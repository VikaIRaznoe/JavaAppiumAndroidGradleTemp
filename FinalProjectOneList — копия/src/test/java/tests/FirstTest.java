...
//        Allure.addAttachment("Результат","text/plain");
    ...
    @Description(value = "Итоговая работа")
    @Epic("Проект #1")
    @Story("Проверка страницы Settings")
    public void goToSettingsPageTest() throws InterruptedException {
        mainPage.goToPageSettings();
        settingsPage.selectPrivateStorage();
        settingsPage.chooseFolderStorageButton();
        settingsPage.allowAccessToYourContent();
        Assert.assertEquals(settingsPage.versionAppСheck(),"1.2.3");
    }
}